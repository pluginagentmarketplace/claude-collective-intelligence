...
// RAMAS Configuration
const RAMAS_REGISTRY_PATH = process.env.RAMAS_REGISTRY_PATH || '/tmp/ramas-windows.json';
const RAMAS_EXCHANGES = {
  STATUS: 'agent.ramas.status',
  INTERRUPT: 'agent.ramas.interrupt',
  PUSH: 'agent.ramas.push'
};
  ...
  return {
    tools: [
      ...
      // === RAMAS (Reactive Agent Messaging & Automation System) ===
      {
        ...
        description: 'Set worker availability status. green=available (will receive messages), red=busy (messages will queue). This updates the terminal title and notifies the RAMAS daemon.',
        inputSchema: {
          type: 'object',
          properties: {
            workerId: {
              type: 'string',
              description: 'Worker ID (e.g., worker-001, worker-002, team-leader)'
            },
            status: {
              type: 'string',
              enum: ['green', 'red'],
              description: 'Availability status: green (available) or red (busy)'
            }
          },
      ...
      },
      {
        ...
        description: 'Send ESC interrupt + message to a worker terminal. For urgent priority, the message is sent immediately even if worker is red. For normal priority, message queues if worker is red.',
        inputSchema: {
          type: 'object',
          properties: {
            workerId: {
            ...
            priority: {
              type: 'string',
              enum: ['normal', 'urgent'],
              description: 'Priority level. urgent = interrupt immediately, normal = respect worker status'
            }
          },
      ...
      },
      {
        ...
        description: 'Get all worker availability statuses from the RAMAS registry. Shows which workers are green (available) or red (busy).',
        inputSchema: {
          type: 'object',
          properties: {}
        }
  ...
  try {
    switch (name) {
      ...
      case 'set_worker_status': {
        ...
        const { workerId, status } = args;

        // Validate status
        if (!['green', 'red'].includes(status)) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ error: 'Status must be "green" or "red"' }) }]
          };
        }

        // Setup RAMAS exchange if not exists
        try {
          await client.channel.assertExchange(RAMAS_EXCHANGES.STATUS, 'fanout', { durable: true });
        } catch (e) {
          // Exchange may already exist
        }

        // Publish status change to RAMAS daemon
        const statusMessage = {
          workerId,
          status,
          changedBy: agentName,
        ...
        client.channel.publish(
          RAMAS_EXCHANGES.STATUS,
          '',
          Buffer.from(JSON.stringify(statusMessage))
        );

        // Also update local registry file directly for immediate feedback
        try {
          let registry = { windows: {} };
          if (fs.existsSync(RAMAS_REGISTRY_PATH)) {
            registry = JSON.parse(fs.readFileSync(RAMAS_REGISTRY_PATH, 'utf8'));
          }
          ...
          fs.writeFileSync(RAMAS_REGISTRY_PATH, JSON.stringify(registry, null, 2));
        } catch (e) {
        ...
        return {
          content: [{ type: 'text', text: JSON.stringify({
            ...
            workerId,
            status,
            message: `Worker ${workerId} status set to ${status.toUpperCase()}`
          }, null, 2) }]
        };
      }
      ...
      case 'interrupt_worker': {
        ...
        const { workerId, message, priority = 'normal' } = args;

        // Setup RAMAS interrupt exchange if not exists
        try {
          await client.channel.assertExchange(RAMAS_EXCHANGES.INTERRUPT, 'direct', { durable: true });
          await client.channel.assertQueue('ramas.interrupts', { durable: true });
          await client.channel.bindQueue('ramas.interrupts', RAMAS_EXCHANGES.INTERRUPT, workerId);
        } catch (e) {
          // May already exist
        }
        ...
        client.channel.publish(
          RAMAS_EXCHANGES.INTERRUPT,
          workerId,
          Buffer.from(JSON.stringify(interruptMessage))
        );
        ...
        return {
          content: [{ type: 'text', text: JSON.stringify({
            ...
            workerId,
            priority,
            message: `Interrupt sent to ${workerId} (${priority})`
          }, null, 2) }]
        };
      }
      ...
      case 'get_worker_statuses': {
        ...
        try {
          if (fs.existsSync(RAMAS_REGISTRY_PATH)) {
            const registry = JSON.parse(fs.readFileSync(RAMAS_REGISTRY_PATH, 'utf8'));
            workers = Object.entries(registry.windows || {}).map(([id, w]) => ({
              workerId: id,
              status: w.status,
              windowId: w.windowId,
              lastStatusChange: w.lastStatusChange
            }));
          }
        } catch (e) {
          return {
            content: [{ type: 'text', text: JSON.stringify({
              error: 'Failed to read RAMAS registry',
              details: e.message
            }) }]
          };
        }
        ...
        return {
          content: [{ type: 'text', text: JSON.stringify({
            registryPath: RAMAS_REGISTRY_PATH,
            workerCount: workers.length,
            workers,
            greenCount: workers.filter(w => w.status === 'green').length,
            redCount: workers.filter(w => w.status === 'red').length
          }, null, 2) }]
        };
      }
